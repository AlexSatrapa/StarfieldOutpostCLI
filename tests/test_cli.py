...
class TestOutpostCommand(unittest.TestCase):
	...
	def test_produces_a_bill_of_materials(self):
		...
		expected = dedent("""
					...
					""")
		self.assertEqual(result.output, expected, "Output should contain appropriate quantity of Isocentered Magnets")

	def test_produces_a_bill_of_materials_optionally_grouped_by_type(self):
		runner: CliRunner = CliRunner()
		result: Result = runner.invoke(cli.outpost, ["bom", "tests/sample_spec.yml", "--group-by-type"])
		expected = dedent("""
					Bill of Materials:
